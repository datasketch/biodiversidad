...
frontmatter$type <- "departamentos"
...
frontmatter$especies_registradas <- reg$especies
...
# Temática
tematica_json_path <- glue::glue("{sel_region}_tematica.json")
jsonlite::write_json(reg,
                     paste0("../data/",tematica_json_path),
...
gr_bio_json_path <- glue::glue("{sel_region}_gr_bio.json")
jsonlite::write_json(gr_bio_sel_region,
                     paste0("../data/",gr_bio_json_path),
                     auto_unbox = TRUE, pretty = TRUE)
...
l
subreg_json_path <- glue::glue("{sel_region}_subreg.json")
jsonlite::write_json(l,
                     paste0("../data/",subreg_json_path),
                     auto_unbox = TRUE, pretty = TRUE)
...
tematica_esp_json_path <- glue::glue("{sel_region}_tematica_esp.json")
jsonlite::write_json(x,
                     paste0("../data/",tematica_esp_json_path),
                     auto_unbox = TRUE, pretty = TRUE)
...
pubs_sel_region <- publicadores |>
  filter(slug_region == sel_region)
pubs_json_path <- glue::glue("{sel_region}_pubs.json")
jsonlite::write_json(pubs_sel_region,
                     paste0("../data/",pubs_json_path),
                     auto_unbox = TRUE, pretty = TRUE)
...
yaml <- paste("---",
      ...
      "---\n\n",
      sep = "\n")
write_lines(yaml, glue::glue("../content/regiones/departamentos/",sel_region,"/_index.md"))
